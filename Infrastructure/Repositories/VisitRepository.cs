...
namespace Infrastructure.Repositories
{
    public class VisitRepository : Repository<Visit>, IVisitRepository
    {
        ...
        {
            return _dbContext.Visits.Where(v => v.VisitDateTime >= startDate && v.VisitDateTime <= endDate).ToList();
        }

        public List<Visit> GetVisitsByDoctorId(int doctorId)
        {
            return _dbContext.Visits.Where(v => v.DoctorId == doctorId).ToList();
        }

        public List<Visit> GetVisitsByPatientId(int patientId)
        {
            return _dbContext.Visits.Where(v => v.PatientId == patientId).ToList();
        }
    }
}
